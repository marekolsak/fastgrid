...
void ParameterLibrary::readParameterLibrary(const char *filename, int outputLevel, int debug, LogFile &logFile)
{
    ...
    while (fgets(line, sizeof(line), parameterLibraryFile) != 0)
        readLine(line, outputLevel, debug, logFile);
}

void ParameterLibrary::setupParameterLibrary(int outputLevel, int debug, LogFile &logFile)
{
    // Default parameters
    ...
    for (int i = 0; param_string[i]; i++)
        readLine(param_string[i], outputLevel, debug, logFile);
}

void ParameterLibrary::readLine(const char *line, int outputLevel, int debug, LogFile &logFile)
{
    ParameterEntry thisParameter;
    int int_hbond_type = 0;
    int nfields;
    int paramKeyword = parseParamLine(line, debug, logFile);
